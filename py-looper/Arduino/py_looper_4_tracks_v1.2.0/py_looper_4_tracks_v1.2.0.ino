 ...
 *  1.2.0)  Added support for UNDO command
 *  
  */
...
const uint8_t btn_hold_cmd[NUM_BUTTONS] = {0, 0, 82, 0, 0, 0, 0, 0, 0};
...
void readButtons()
{
  ...
  for( int i=0;i<NUM_BUTTONS;i++){
     if (btnEval[i] == 0 && retriggerTimeExpired(btnLastPressTime[i])){
        ...
        //handle long press
        }else if((millis() - btnLastPressTime[i]) >= longPressTime){
              //check if "hold cmd" is not 0.  If so then send midi, otherwise do nothing
              if(btn_hold_cmd[i] != 0){
                  sendCmd(1, btn_hold_cmd[i], 127);
              }
              btnLastPressTime[i] = millis();
        }
     }
